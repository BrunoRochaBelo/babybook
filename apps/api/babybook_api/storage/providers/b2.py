...
B2 é usado como cold storage para:
- Originais de alta resolução
- Vídeos (arquivos grandes)
- Backup e arquivamento de longo prazo

Vantagens:
- Custo de storage muito baixo
- Bandwidth Alliance com Cloudflare (egress gratuito via CF)
- Durabilidade 11 nines
"""
...
class B2Provider(StorageProvider):
    """
    Provider para Backblaze B2 (S3-compatible API).
    ...
    async def initialize(self) -> None:
        if aioboto3 is None:
            raise ImportError("aioboto3 é necessário para B2Provider. Instale com: pip install aioboto3")
