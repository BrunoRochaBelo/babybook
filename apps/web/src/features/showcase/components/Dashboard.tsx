...
import { Chapter, chapters, getMomentsByChapter } from "../lib/chaptersData";
...
export function Dashboard({ babyName, onSelectChapter, onNavigate, onSettings }: DashboardProps) {
  ...
  const chaptersWithProgress: Array<Chapter & { progress: number; total: number }> = chapters.map(chapter => {
    ...
    <div className="min-h-screen bg-background pb-32">
      ...
      <div className="max-w-4xl mx-auto px-4 py-6 sm:py-8">
        ...
        >
          ...
          {chaptersWithProgress.every(c => c.progress === 0) && (
            <motion.div
              initial={{ opacity: 0 }}
              animate={{ opacity: 1 }}
