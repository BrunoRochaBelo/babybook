...
const SCREEN_SEQUENCE: Array<{ id: Screen; label: string }> = [
...
export function CaraCrachaExperience() {
  ...
  const handleNavigate = (section: 'memories' | 'health' | 'visits' | 'guestbook') => {
    if (section === 'memories') {
      setCurrentScreen('dashboard');
  ...
  return (
    <ThemeProvider>
      <div className="pointer-events-none fixed right-4 top-4 z-[60] hidden xl:block">
        <div className="pointer-events-auto rounded-2xl border border-slate-200/80 bg-white/80 p-4 shadow-2xl backdrop-blur dark:border-slate-700/60 dark:bg-slate-900/80">
          <p className="text-[10px] font-semibold uppercase tracking-[0.2em] text-slate-500 dark:text-slate-300">
            Cara-crachá
          </p>
          <div className="mt-3 flex flex-wrap gap-2">
            {SCREEN_SEQUENCE.map((screen) => (
              <button
                ...
                className={`rounded-full border px-3 py-1 text-xs font-medium transition hover:shadow ${
      ...
      <div className="fixed inset-x-4 bottom-4 z-[60] xl:hidden">
        <label className="mb-1 block text-xs font-medium text-slate-600 dark:text-slate-300">
          Cara-crachá - escolha uma tela
        </label>
        <div className="flex items-center gap-2 rounded-2xl border border-slate-200/80 bg-white/90 px-3 py-2 shadow-lg backdrop-blur dark:border-slate-700/60 dark:bg-slate-900/90">
          <select
            value={currentScreen}
            onChange={(event) => setCurrentScreen(event.target.value as Screen)}
            className="flex-1 rounded-xl border border-transparent bg-transparent text-sm font-medium text-slate-700 focus:border-brand-500 focus:outline-none dark:text-slate-200"
          >
            {SCREEN_SEQUENCE.map((screen) => (
